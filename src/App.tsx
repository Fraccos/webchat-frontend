...
function App() {
	...
	const setPalette = (mode: PaletteMode) => {
		if (mode === "light") return lightPalette as ThemeOptions;
		if (mode === "dark") return darkPalette as ThemeOptions;
	};

	const Theme = createTheme(setPalette(mode), [mode]);
    ...
    perchè sia meglio passarlo come body e non come URLparams quindi opto per una posto */
		if (missingUserSet.size > 0) {
			cAPIWrapper
				.post("/users/usernamesMap/retrive", {
					data: {
		...
		let content =
			newChatroom.type === "group" ? (
				<>
					...
					<strong>{newChatroom.name}</strong>
				</>
			) : (
				<>
					<strong>{newChatroom.name}</strong> ha iniziato una
	...
	const onChatroomDeleted = (data: any) => {
		...
		setChatrooms((prev) => {
			const _del = prev.find(
				(c) => c._id.toString() === delChatroom.id.toString()
			);
			NotificationService.push({
				type: "warning",
				content: (
					<>
						La chatroom <strong>{_del?.name}</strong> è stata
						eliminata
					</>
				),
				insertionDate: new Date(),
	...
	const onNewFriendshipReq = (data: any) => {
		...
		NotificationService.push({
			...
			content: (
				<>
					Hai ricevuto una nuova richiesta di amicizia da{" "}
					{frReq.receiver.username}
				</>
			),
			insertionDate: new Date(),
		});
	...
	const onNewFriend = (data: any) => {
		...
		const newFriend: User =
			fR.receiver._id?.toString() === currentUser?._id?.toString()
				? fR.sender
				: fR.receiver;
		if (fR.receiver._id?.toString() == currentUser?._id?.toString()) {
			...
			NotificationService.push({
				...
				content: (
					<>
						{fR.receiver.username} ha accettato la tua richiesta di
						amicizia
					</>
				),
				insertionDate: new Date(),
		...
		if (user._id !== undefined) {
			...
			if (data.type === "removed") {
				NotificationService.push({
					...
					content: (
						<>
							L'utente {user.username} ti ha rimosso dai suoi
							amici
						</>
					),
					insertionDate: new Date(),
				...
				NotificationService.push({
					...
					content: (
						<>
							Hai rimosso con successo dai tuoi amici{" "}
							{user.username}
						</>
					),
					insertionDate: new Date(),
	...
	const onUpdatedLastRead = (data: any) => {
		...
		setChatrooms((oldChatrooms: Chatroom[]) => {
	...
	useEffect(() => {
		if (JWT.length > 0) {
			...
			cAPIWrapper
				...
				.then(() =>
					cAPIWrapper.get(
						`/users/friends/retrive/${currentUser?._id}`
					)
				)
				.then((res) => {
					setFriends(res.data);
		...
		<ThemeProvider theme={Theme}>
			...
			<>
				<div className="App">
					...
					<Snackbar
						open={showNotication}
						onClose={() => setShowNotification((prev) => !prev)}
						anchorOrigin={{
							vertical: "bottom",
							horizontal: "right",
						}}
